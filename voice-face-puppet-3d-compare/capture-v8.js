...
let running=false,stream=null,ctx=null,source=null,wlNode=null,haNode=null,rawOnset=null,recorder=null;
...
let haRawActive='sil',lastVadDb=-100,latestGate=null,latestRealism=null,latestWl=null,latestRawOnset={level:0,db:-120,floorDb:-55};
...
function onsetSnapshot(){return {level:+(latestRawOnset.level||0).toFixed(5),db:+(latestRawOnset.db??-120).toFixed(3),floorDb:+(latestRawOnset.floorDb??-55).toFixed(3)}}
...
function setPhrase(id){
  ...
  status(id==='connected'?'Ready for connected-speech V8 recording.':'Ready for original phoneme-stress V8 recording.');
}
...
async function startCapture(){
  ...
  $('recordBtn').disabled=true;$('downloadCaptureBtn').disabled=true;document.querySelectorAll('[data-diagnostic-phrase]').forEach(b=>b.disabled=true);status('Preparing V11 realism + raw-onset capture…');setProgress(0);
  try{
    ...
    stream=await navigator.mediaDevices.getUserMedia({audio:{echoCancellation:true,noiseSuppression:true,autoGainControl:true}});ctx=new AudioContext();await ctx.resume();source=ctx.createMediaStreamSource(stream);rawOnset=new RawOnsetFollower(ctx,source);
    ...
    chunks=[];frames=[];features=[];visemeEvents=[];lastVadDb=-100;haRawActive='sil';gate.reset();latestGate=gate.last;realismEngine.reset();latestRealism=null;latestWl=null;latestRawOnset={level:0,db:-120,floorDb:-55};for(const k of Object.keys(haRaw))haRaw[k]=0;
    ...
    t0=performance.now();lastUpdate=t0;running=true;recorder.start(200);$('recordBtn').disabled=false;$('recordBtn').textContent='STOP NOW';$('recordBtn').classList.add('recording');status('RECORDING V8 REALISM — read the displayed phrase naturally.');
    updateTimer=setInterval(()=>{
      const now=performance.now(),dt=Math.min(50,now-lastUpdate);lastUpdate=now;haNode?.update(dt);latestRawOnset=rawOnset?.sample(dt,{speechHint:lastVadDb>-55})||{level:0,db:-120,floorDb:-55};latestGate=gate.update(haRaw,lastVadDb,haRawActive,now);latestWl=wlSnapshot();latestRealism=realismEngine.update({vowelWeights:latestWl.weights,volume:latestWl.rawVolume,haWeights:haRaw,rawActive:haRawActive,gate:latestGate,vadDb:lastVadDb,rawOnset:latestRawOnset.level,dtMs:dt});
    },16);
    timer=setInterval(()=>{
      const now=performance.now(),elapsed=now-t0,wl=latestWl||wlSnapshot(),raw=rawSnapshot(),g=gateSnapshot(),onset=onsetSnapshot(),realism=latestRealism||realismEngine.update({vowelWeights:wl.weights,volume:wl.rawVolume,haWeights:haRaw,rawActive:haRawActive,gate:latestGate,vadDb:lastVadDb,rawOnset:onset.level,dtMs:16});
      const r={shape:realism.shape,detail:realism.detail,level:+realism.level.toFixed(5),rawOnset:+(realism.rawOnset||0).toFixed(5),speechEnvelope:+realism.speechEnvelope.toFixed(5),morphs:compactMorphs(realism.morphs),overlay:realism.overlay?{active:realism.overlay.active,strength:+realism.overlay.strength.toFixed(5),all:Object.fromEntries(Object.entries(realism.overlay.all||{}).map(([k,v])=>[k,+v.toFixed(5)]))}:null};
      frames.push({t:+elapsed.toFixed(1),rawOnset:onset,wl,headAudioRaw:raw,strongGate:g,realism:r});setProgress(elapsed/12000);status(`RECORDING V8 — ${(elapsed/1000).toFixed(1)} / 12.0 s · onset ${Math.round(onset.level*100)}% · ${r.detail}`)
    },33);
...
async function stopCapture(commit){
  ...
  try{
    ...
    if(commit){
      ...
      const bundle={format:'voice-face-lipsync-diagnostic-v8-realism',createdAt:new Date().toISOString(),phraseId,sentence,expectedWords:sentence.replace(/[.,’']/g,'').split(/\s+/),durationMs:+duration.toFixed(1),audio:{mimeType:audioBlob.type,size:audioBlob.size,dataUrl:audioDataUrl},models:{wLipSync:{personalProfile:!!wlSaved,smoothness:.020},headAudio:{commit:HA_COMMIT,personalSpeechPrototypeCount:Object.keys(personalConsonants).length,personalClasses:Array.from(PERSONAL_CLASSES),personalVowelsLoaded:false},strongAnchorGate:{speechGateDb:gate.speechGateDb,classes:CONSONANTS,config:GATE_CONFIG},rawOnset:{module:'raw-onset-v1.js',adaptiveNoiseFloor:true,fastAttackMs:8},realismEngine:{module:'articulation-engine-v2.js',articulation:.70,continuousAllConsonants:true,separateJawLips:true,predictiveLead:true,rawOnsetWake:true,fastTrueSilenceRelease:true}},sampling:{frameIntervalTargetMs:33,engineUpdateTargetMs:16,gateUpdateTargetMs:16,featureVectors:'HeadAudio 12D MFCC events'},frames,features,visemeEvents};
      makeDownload(bundle);const overlays=frames.filter(f=>f.realism.overlay).length;status(`V8 captured ${(duration/1000).toFixed(1)} s · ${frames.length} realism frames · ${features.length} features · ${overlays} articulated frames. Download and upload this JSON.`);setProgress(1)
    }
  }catch(e){console.error(e);status(`Could not package V8 recording: ${e.message}`)}finally{cleanup();$('recordBtn').disabled=false;$('recordBtn').textContent='RECORD 12s';document.querySelectorAll('[data-diagnostic-phrase]').forEach(b=>b.disabled=false)}
}
function cleanup(){running=false;clearInterval(timer);clearInterval(updateTimer);clearTimeout(autoStopTimer);timer=updateTimer=autoStopTimer=null;try{rawOnset?.disconnect()}catch{};rawOnset=null;try{source?.disconnect()}catch{};try{wlNode?.disconnect()}catch{};try{haNode?.disconnect()}catch{};stream?.getTracks().forEach(t=>t.stop());stream=source=wlNode=haNode=null;ctx?.close().catch(()=>{});ctx=null;recorder=null;gate.reset();latestGate=gate.last;latestRealism=null;latestWl=null;latestRawOnset={level:0,db:-120,floorDb:-55};realismEngine.reset()}
