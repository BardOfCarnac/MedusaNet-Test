...
import { ConsonantGate, CONSONANTS } from './hybrid-gate.js';
...
async function bootHead(){try{setStatus('LOADING NEUTRAL / ~2.6 MB');prepareBase(await loadObj('generic_neutral_mesh.obj'));for(let i=0;i<MORPHS.length;i++){setStatus(`LOADING ARKIT MOUTH MORPH ${i+1}/${MORPHS.length}`);attachTarget(await loadObj(MORPHS[i]),i);await new Promise(r=>requestAnimationFrame(r))}buildHead();faceReady=true;setStatus('HYBRID HEAD READY / WAITING FOR MIC','ready')}catch(e){console.error(e);setStatus(`HEAD LOAD FAILED / ${e?.message||'UNKNOWN'}`,'error')}}
...
async function startMic(){
  ...
  try{
    ...
    wlNode=await wlLib.createWLipSyncNode(audioCtx,wlSaved||stockWlProfile);wlNode.smoothness=.055;source.connect(wlNode);
...
function hybridState(now){
  ...
  morphs.mouthPucker=Math.min(.40,morphs.mouthPucker||0);
...
function applyMorphState(target){
  for(const k of MORPH_KEYS)morph[k]=lerp(morph[k],target.morphs[k]||0,.30);
