import { ConsonantGate, GATE_CONFIG, CONSONANTS } from './hybrid-gate-v2.js';
...
async function startCapture(){
  ...
  try{
    ...
    t0=performance.now();lastUpdate=t0;running=true;recorder.start(200);$('recordBtn').disabled=false;$('recordBtn').textContent='STOP NOW';$('recordBtn').classList.add('recording');status('RECORDING V5 HYBRID — read the sentence naturally.');
    ...
    timer=setInterval(()=>{const now=performance.now(),elapsed=now-t0;const wl=wlSnapshot(),raw=rawSnapshot(),g=gateSnapshot();frames.push({t:+elapsed.toFixed(1),wl,headAudioRaw:raw,hybrid:{baseVowel:wl.shape,consonant:g.active,strength:g.strength,gate:g}});setProgress(elapsed/12000);status(`RECORDING V5 — ${(elapsed/1000).toFixed(1)} / 12.0 s · ${wl.shape.toUpperCase()} + ${g.active||'—'} ${Math.round(g.strength*100)}%`)},33);
