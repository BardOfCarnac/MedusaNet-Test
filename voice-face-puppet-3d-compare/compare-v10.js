...
async function bootHead(){try{setStatus('LOADING NEUTRAL / ~2.6 MB');prepareBase(await loadObj('generic_neutral_mesh.obj'));for(let start=0;start<MORPHS.length;start+=4){const end=Math.min(start+4,MORPHS.length);setStatus(`LOADING FACE MORPHS ${start+1}–${end}/${MORPHS.length}`);const roots=await Promise.all(MORPHS.slice(start,end).map(loadObj));roots.forEach((root,offset)=>attachTarget(root,start+offset));await new Promise(r=>requestAnimationFrame(r))}buildHead();faceReady=true;setStatus('REALISM SPEECH + EXPRESSION DECK READY','ready')}catch(e){console.error(e);setStatus(`HEAD LOAD FAILED / ${e?.message||'UNKNOWN'}`,'error')}}
...
function updateProfileInfo(){if($('profileInfo'))$('profileInfo').textContent=`wLipSync: ${wlSaved?'PERSONAL':'stock'} · realism articulation: 5 continuous vowel fields + 9 consonant fields · ${Object.keys(personalConsonants).length} personal high-visibility consonant Gaussians`}
updateProfileInfo();
...
async function startMic(){
  ...
  try{
    ...
    source.connect(haNode);mic=true;$('micBtn').textContent='MIC OFF';$('micBtn').classList.add('selected');$('micStatus').textContent='Realism live: wLipSync supplies overlapping vowel evidence; HeadAudio continuously influences all consonant classes, with visible anchors stronger than internal articulation.';setStatus('REALISM LIP SYNC LIVE','ready');
...
function wLipState(now,dtMs){
  const src=lipInput();
  return vowelEngine.update({vowelWeights:src.weights,volume:src.volume,dtMs,vadDb:lastVadDb,haWeights:{},rawActive:'sil',gate:null});
}
function hybridState(now,dtMs){
  const src=lipInput(),gate=consonantGate.update(haRaw,lastVadDb,haRawActive,now);
  const out=hybridEngine.update({vowelWeights:src.weights,volume:src.volume,haWeights:haRaw,rawActive:haRawActive,gate,vadDb:lastVadDb,dtMs});
...
function stopDiagnosticReplay(){diagnosticReplay.active=false;diagnosticReplay.index=0;const b=$('replayBtn');if(b){b.textContent='REPLAY DIAGNOSTIC · 0.55×';b.classList.remove('selected')}if(faceReady)setStatus(mic?'REALISM LIP SYNC LIVE':'REALISM HEAD READY / WAITING FOR MIC','ready')}
...
function applyMorphState(target,dtMs=16){
  ...
  $('meterFill').style.width=`${clamp(target.level)*100}%`;$('shapeOut').textContent=target.detail||target.shape;$('hudShape').textContent=String(target.shape||'—').toUpperCase();$('hudTemporal').textContent=active==='hybrid'?(target.overlay?.active?`${target.overlay.active} ${Math.round(target.overlay.strength*100)}% · overlap`:`HA ${haRawActive.toUpperCase()} · continuous`):'5 VOWELS · OVERLAP';
