...
// ICT FaceKit ARKit-style morphs used by the TalkingHead Oculus-viseme recipes.
// Source recipe: met4citizen/TalkingHead, blender/build-visemes-from-arkit.py (MIT).
// FaceKit does not expose ARKit tongueOut, so the TH/nn tongue component is intentionally omitted.
const MORPH_FILES={
  jawOpen:'jawOpen.obj',
  jawForward:'jawForward.obj',
  mouthFunnel:'mouthFunnel.obj',
  mouthPucker:'mouthPucker.obj',
  mouthPress_L:'mouthPress_L.obj',
  mouthPress_R:'mouthPress_R.obj',
  mouthDimple_L:'mouthDimple_L.obj',
  mouthDimple_R:'mouthDimple_R.obj',
  mouthRollLower:'mouthRollLower.obj',
  mouthRollUpper:'mouthRollUpper.obj',
  mouthUpperUp_L:'mouthUpperUp_L.obj',
  mouthUpperUp_R:'mouthUpperUp_R.obj',
  mouthShrugUpper:'mouthShrugUpper.obj',
  mouthLowerDown_L:'mouthLowerDown_L.obj',
  mouthLowerDown_R:'mouthLowerDown_R.obj',
  // Kept for the original wLipSync vowel comparison.
  mouthStretch_L:'mouthStretch_L.obj',
  mouthStretch_R:'mouthStretch_R.obj'
};
const MORPH_KEYS=Object.keys(MORPH_FILES);
const MORPHS=MORPH_KEYS.map(k=>MORPH_FILES[k]);
const MI=Object.fromEntries(MORPH_KEYS.map((k,i)=>[k,i]));

const VISEME_RECIPES={
  aa:{jawOpen:.6},
  E:{mouthPress_L:.8,mouthPress_R:.8,mouthDimple_L:1,mouthDimple_R:1,jawOpen:.3},
  I:{mouthPress_L:.6,mouthPress_R:.6,mouthDimple_L:.6,mouthDimple_R:.6,jawOpen:.2},
  O:{mouthPucker:1,jawForward:.6,jawOpen:.2},
  U:{mouthFunnel:1},
  ...
  DD:{mouthPress_L:.8,mouthPress_R:.8,mouthFunnel:.5,jawOpen:.2},
  SS:{mouthPress_L:.8,mouthPress_R:.8,mouthLowerDown_L:.5,mouthLowerDown_R:.5,jawOpen:.1},
  TH:{mouthRollUpper:.6,jawOpen:.2}, // TalkingHead also uses tongueOut .4.
  CH:{mouthPucker:.5,jawOpen:.2},
  RR:{mouthPucker:.5,jawOpen:.2},
  kk:{mouthLowerDown_L:.4,mouthLowerDown_R:.4,mouthDimple_L:.3,mouthDimple_R:.3,mouthFunnel:.3,mouthPucker:.3,jawOpen:.15},
  nn:{mouthLowerDown_L:.4,mouthLowerDown_R:.4,mouthDimple_L:.3,mouthDimple_R:.3,mouthFunnel:.3,mouthPucker:.3,jawOpen:.15}, // TalkingHead also uses tongueOut .2.
  sil:{}
};
...
async function bootHead(){
  try{
    ...
    for(let i=0;i<MORPHS.length;i++){
      setStatus(`LOADING ARKIT MOUTH MORPH ${i+1}/${MORPHS.length}`);
      attachTarget(await loadObj(MORPHS[i]),i);
      await new Promise(r=>requestAnimationFrame(r));
    }
    buildHead();faceReady=true;
    setStatus('ARKIT VISEME HEAD READY / WAITING FOR MIC','ready');
  }catch(e){console.error(e);setStatus(`HEAD LOAD FAILED / ${e?.message||'UNKNOWN'}`,'error')}
}
...
function updateProfileInfo(){
  $('profileInfo').textContent=`wLipSync: ${wlSaved?'PERSONAL':'stock'} · HeadAudio: ${Object.keys(personal.prototypes).length} Gaussian + ${Object.keys(temporal.templates).length} temporal · TalkingHead ARKit recipes`;
}
updateProfileInfo();
...
async function startMic(){
  ...
  try{
    ...
    $('micStatus').textContent='Both trackers are live; HeadAudio now drives the full TalkingHead ARKit viseme recipe.';
    setStatus('HEAD + ARKIT VISEMES LIVE','ready');
  }catch(e){
...
function stopMic(update=true){
  ...
  if(faceReady)setStatus('ARKIT VISEME HEAD READY / WAITING FOR MIC','ready');
}
...
function wLipState(){
  const morphs=blankMorphs();
  if(!wlNode)return {morphs,shape:'rest',level:0,temporal:null};
  ...
  morphs.jawOpen=clamp(o*v*1.05);
  morphs.mouthFunnel=clamp(funnel*v);
  morphs.mouthPucker=clamp(pucker*v);
  morphs.mouthStretch_L=morphs.mouthStretch_R=clamp(stretch*v);
  return {morphs,shape:v>.04?shape:'rest',level:v,temporal:null};
}
function recipeMorphs(weights){
  const out=blankMorphs();
  for(const [viseme,recipe] of Object.entries(VISEME_RECIPES)){
    const vw=clamp(weights[`viseme_${viseme}`]||0);
    if(vw<=0)continue;
    for(const [morphName,amount] of Object.entries(recipe)){
      if(morphName in out)out[morphName]+=vw*amount;
    }
  }
  // Multiple HeadAudio viseme probabilities can overlap; FaceKit morph targets are 0..1.
  for(const k of MORPH_KEYS)out[k]=clamp(out[k]);
  return out;
}
function headAudioState(now){
  if(!haNode)return {morphs:blankMorphs(),shape:'sil',level:0,temporal:null};
  const o=composeHeadAudio(now);
  return {morphs:recipeMorphs(o.weights),shape:o.shape,level:clamp((lastVadDb+65)/38),temporal:o.temporal};
}
...
function selectModel(which){
  ...
  $('activeModel').textContent=which==='wl'?'wLipSync':'HeadAudio + ARKit';$('hudModel').textContent=which==='wl'?'WLIPSYNC':'HEADAUDIO / ARKIT';
}
function aggregate(m,a,b=null){return b?clamp(((m[a]||0)+(m[b]||0))*.5):clamp(m[a]||0)}
function setBar(id,value){const el=$(id);if(el)el.style.width=`${clamp(value)*100}%`}
function applyMorphState(target){
  const speed=active==='ha'?.34:.30;
  const tm=target.morphs||{};
  for(const k of MORPH_KEYS)morph[k]=lerp(morph[k],tm[k]||0,speed);
  if(faceReady){
    for(const {solid,wire} of renderMeshes){
      for(const m of [solid,wire]){
        if(!m.morphTargetInfluences)continue;
        for(const k of MORPH_KEYS)m.morphTargetInfluences[MI[k]]=morph[k];
      }
      wire.visible=$('wire').checked;
    }
  }
  setBar('mJaw',morph.jawOpen);
  setBar('mFunnel',morph.mouthFunnel);
  setBar('mPucker',morph.mouthPucker);
  setBar('mPress',aggregate(morph,'mouthPress_L','mouthPress_R'));
  setBar('mDimple',aggregate(morph,'mouthDimple_L','mouthDimple_R'));
  setBar('mRoll',Math.max(morph.mouthRollLower||0,morph.mouthRollUpper||0));
  setBar('mLower',aggregate(morph,'mouthLowerDown_L','mouthLowerDown_R'));
  setBar('mUpper',aggregate(morph,'mouthUpperUp_L','mouthUpperUp_R'));
  setBar('mForward',morph.jawForward);
  setBar('mStretch',aggregate(morph,'mouthStretch_L','mouthStretch_R'));
  $('meterFill').style.width=`${clamp(target.level)*100}%`;
  $('shapeOut').textContent=String(target.shape||'—').toUpperCase();$('hudShape').textContent=String(target.shape||'—').toUpperCase();$('hudTemporal').textContent=target.temporal?`TEMP ${target.temporal}`:'TEMP —';
  window.LipSync3DOutput={model:active,shape:target.shape,morphs:{...morph},level:target.level,temporal:target.temporal||null,recipe:active==='ha'?'TalkingHead ARKit→Oculus':null};
}
