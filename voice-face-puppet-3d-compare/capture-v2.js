...
let chunks=[],frames=[],features=[],events=[],candidates=[],timer=null,updateTimer=null,autoStopTimer=null,t0=0,lastUpdate=0;
...
let haRawActive=14,lastVadDb=-100,featureHistory=[],override=null,lastTriggerAt=-Infinity,lastCandidateAt=-Infinity;
...
function compactWeights(obj){const o={};for(const n of HA_NAMES)o[n]=+(obj[`viseme_${n}`]||0).toFixed(4);return o}
...
function scoreTemporal(d){const scores=[];for(const [key,q] of Object.entries(temporal.templates)){if(!q?.mean||!q?.variance||!Number.isFinite(q.threshold))continue;const score=distDesc(d,q.mean,q.variance),ratio=score/q.threshold;scores.push({key,label:q.label,visemeIndex:q.visemeIndex,score,ratio})}scores.sort((a,b)=>a.ratio-b.ratio);return scores}
function evaluateTemporal(){
  ...
  const now=performance.now();if(now-lastCandidateAt<110)return;lastCandidateAt=now;
  const d=descriptor(r,i);if(!d)return;
  const scores=scoreTemporal(d);if(!scores.length)return;
  const best=scores[0],second=scores[1]||null,margin=second&&best.ratio>0?second.ratio/best.ratio:null;
  if(running)candidates.push({t:+(now-t0).toFixed(1),db:+db.toFixed(3),rise:+rise.toFixed(3),best:{key:best.key,label:best.label,viseme:HA_NAMES[best.visemeIndex]||null,ratio:+best.ratio.toFixed(4),score:+best.score.toFixed(4)},second:second?{key:second.key,label:second.label,viseme:HA_NAMES[second.visemeIndex]||null,ratio:+second.ratio.toFixed(4),score:+second.score.toFixed(4)}:null,margin:margin?+margin.toFixed(4):null,scores:scores.map(s=>({key:s.key,label:s.label,ratio:+s.ratio.toFixed(4),score:+s.score.toFixed(4)}))});
  if(best.ratio<=1&&now-lastTriggerAt>=110){lastTriggerAt=now;override={visemeIndex:best.visemeIndex,label:best.label,until:now+90,ratio:best.ratio};if(running)events.push({t:+(now-t0).toFixed(1),type:'temporal',label:best.label,viseme:HA_NAMES[best.visemeIndex]||null,ratio:+best.ratio.toFixed(4),margin:margin?+margin.toFixed(4):null})}
}
...
async function startCapture(){
  ...
  try{
    ...
    frames=[];features=[];events=[];candidates=[];chunks=[];featureHistory=[];override=null;lastTriggerAt=-Infinity;lastCandidateAt=-Infinity;lastVadDb=-100;haRawActive=14;for(const k of HA_KEYS)haRaw[k]=haFinal[k]=0;
    ...
    t0=performance.now();lastUpdate=t0;running=true;recorder.start(200);$('recordBtn').disabled=false;$('recordBtn').textContent='STOP NOW';$('recordBtn').classList.add('recording');status('RECORDING V2 — read the sentence naturally.');
    updateTimer=setInterval(()=>{const now=performance.now(),dt=Math.min(50,now-lastUpdate);lastUpdate=now;haNode?.update(dt)},16);
    timer=setInterval(()=>{const now=performance.now(),elapsed=now-t0;frames.push({t:+elapsed.toFixed(1),wl:wlSnapshot(),headAudio:haSnapshot(now)});setProgress(elapsed/12000);status(`RECORDING V2 — ${(elapsed/1000).toFixed(1)} / 12.0 s · candidate scores ${candidates.length}`)},33);
