...
function enableMorphControls(){document.querySelectorAll('.identity input,#jaw,#expressionStrength').forEach(el=>el.disabled=false);ready=true;setStatus(`READY / ${currentMask.toUpperCase()} MASK`,'ready');setExpression('neutral',false);syncMorphs();}
...
function setExpression(name,announce=true){
  currentExpression=EXPRESSION_PRESETS[name]?name:'neutral';expressionTarget.fill(0);
  const strength=Number(ui.expressionStrength?.value||0);
  Object.entries(EXPRESSION_PRESETS[currentExpression]).forEach(([shape,value])=>{const index=MORPH_INDEX[shape];if(index!==undefined)expressionTarget[index]=value*strength;});
...
ui.expressionStrength.addEventListener('input',()=>{document.querySelector('#expressionStrengthOut').value=pct(ui.expressionStrength.value);setExpression(currentExpression,false);});
