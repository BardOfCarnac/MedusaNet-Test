...
const EXPRESSION_TARGETS=[
  ...
  'browInnerUp_L.obj','browInnerUp_R.obj','browOuterUp_L.obj','browOuterUp_R.obj','browDown_L.obj','browDown_R.obj',
  'cheekRaiser_L.obj','cheekRaiser_R.obj','noseSneer_L.obj','noseSneer_R.obj',
  'mouthSmile_L.obj','mouthSmile_R.obj','mouthFrown_L.obj','mouthFrown_R.obj','mouthPress_L.obj','mouthPress_R.obj',
  'mouthDimple_L.obj','mouthDimple_R.obj','mouthStretch_L.obj','mouthStretch_R.obj',
  'mouthShrugUpper.obj','mouthShrugLower.obj','mouthUpperUp_L.obj','mouthUpperUp_R.obj','mouthLowerDown_L.obj','mouthLowerDown_R.obj','mouthRollUpper.obj','mouthRollLower.obj'
];
...
const card=(label,weights={})=>({label,weights});
const FAMILY_DECKS={
  attentive:{
    brows:[card('quiet inner lift',{browInnerUp_L:.18,browInnerUp_R:.18}),card('outer attention',{browOuterUp_L:.17,browOuterUp_R:.17}),card('left brow leads',{browInnerUp_L:.22,browOuterUp_R:.10}),card('brows remain still')],
    eyes:[card('soft focus',{eyeSquint_L:.09,eyeSquint_R:.09}),card('eyes open slightly',{eyeWide_L:.14,eyeWide_R:.14}),card('one eye considers',{eyeSquint_L:.14,eyeSquint_R:.05}),card('eyes remain clear')],
    mouth:[card('lips gather',{mouthPress_L:.10,mouthPress_R:.10}),card('almost a smile',{mouthSmile_L:.10,mouthSmile_R:.07}),card('upper lip attends',{mouthShrugUpper:.10}),card('mouth remains neutral')]
  },
  warm:{
    brows:[card('open brows',{browOuterUp_L:.13,browOuterUp_R:.13}),card('friendly inner lift',{browInnerUp_L:.12,browInnerUp_R:.12}),card('one brow welcomes',{browOuterUp_L:.16,browInnerUp_R:.07}),card('brows remain soft')],
    eyes:[card('cheeks rise',{cheekRaiser_L:.18,cheekRaiser_R:.18}),card('smiling squint',{eyeSquint_L:.16,eyeSquint_R:.16}),card('uneven warmth',{cheekRaiser_L:.20,cheekRaiser_R:.11}),card('eyes remain open')],
    mouth:[card('balanced smile',{mouthSmile_L:.34,mouthSmile_R:.34}),card('crooked smile',{mouthSmile_L:.40,mouthSmile_R:.20}),card('contained dimple',{mouthDimple_L:.30,mouthDimple_R:.22}),card('private smile',{mouthSmile_L:.19,mouthSmile_R:.14,mouthPress_L:.07,mouthPress_R:.07})]
  },
  sceptical:{
    brows:[card('left brow challenges',{browDown_L:.34,browInnerUp_R:.24}),card('right brow challenges',{browDown_R:.34,browInnerUp_L:.24}),card('brows narrow',{browDown_L:.23,browDown_R:.23}),card('outer brow question',{browOuterUp_L:.24,browDown_R:.16})],
    eyes:[card('left eye narrows',{eyeSquint_L:.27,eyeSquint_R:.08}),card('right eye narrows',{eyeSquint_R:.27,eyeSquint_L:.08}),card('measured squint',{eyeSquint_L:.18,eyeSquint_R:.18}),card('small sneer',{noseSneer_L:.16,noseSneer_R:.08})],
    mouth:[card('one-sided press',{mouthPress_L:.25,mouthPress_R:.10}),card('mouth pulls aside',{mouthStretch_L:.21,mouthStretch_R:.08}),card('doubtful corner',{mouthFrown_R:.20,mouthPress_L:.10}),card('restrained dimple',{mouthDimple_L:.17,mouthPress_R:.14})]
  },
  concerned:{
    brows:[card('inner brows rise',{browInnerUp_L:.45,browInnerUp_R:.45}),card('uneven worry',{browInnerUp_L:.48,browInnerUp_R:.30}),card('brow pinch',{browInnerUp_L:.30,browInnerUp_R:.30,browDown_L:.10,browDown_R:.10}),card('one brow searches',{browInnerUp_R:.40,browOuterUp_L:.12})],
    eyes:[card('eyes search',{eyeWide_L:.19,eyeWide_R:.19}),card('painful squint',{eyeSquint_L:.15,eyeSquint_R:.15}),card('uneven uncertainty',{eyeWide_L:.18,eyeSquint_R:.09}),card('cheeks tense',{cheekRaiser_L:.10,cheekRaiser_R:.10})],
    mouth:[card('corners fall',{mouthFrown_L:.23,mouthFrown_R:.23}),card('lower lip worries',{mouthShrugLower:.18}),card('one corner falls',{mouthFrown_L:.25,mouthFrown_R:.10}),card('lips part downward',{mouthLowerDown_L:.14,mouthLowerDown_R:.14,mouthPress_L:.06,mouthPress_R:.06})]
  },
  guarded:{
    brows:[card('brows settle',{browDown_L:.16,browDown_R:.16}),card('one brow locks',{browDown_L:.20,browDown_R:.10}),card('inner tension',{browInnerUp_L:.10,browInnerUp_R:.10,browDown_L:.11,browDown_R:.11}),card('brows reveal nothing')],
    eyes:[card('eyes narrow',{eyeSquint_L:.14,eyeSquint_R:.14}),card('side tension',{eyeSquint_L:.18,eyeSquint_R:.08}),card('cheeks brace',{cheekRaiser_L:.09,cheekRaiser_R:.09}),card('eyes remain unreadable')],
    mouth:[card('lips seal',{mouthPress_L:.27,mouthPress_R:.27}),card('upper lip rolls in',{mouthRollUpper:.22,mouthPress_L:.09,mouthPress_R:.09}),card('lower lip rolls in',{mouthRollLower:.20,mouthPress_L:.11,mouthPress_R:.11}),card('one side locks',{mouthPress_L:.29,mouthPress_R:.13})]
  },
  activated:{
    brows:[card('brows spring open',{browInnerUp_L:.38,browInnerUp_R:.38,browOuterUp_L:.28,browOuterUp_R:.28}),card('one brow fires first',{browOuterUp_L:.40,browInnerUp_R:.27}),card('central alarm',{browInnerUp_L:.48,browInnerUp_R:.48}),card('sharp outer lift',{browOuterUp_L:.38,browOuterUp_R:.38})],
    eyes:[card('eyes widen',{eyeWide_L:.50,eyeWide_R:.50}),card('uneven startle',{eyeWide_L:.55,eyeWide_R:.35}),card('tense activation',{eyeWide_L:.30,eyeWide_R:.30,cheekRaiser_L:.10,cheekRaiser_R:.10}),card('alert squint',{eyeSquint_L:.16,eyeSquint_R:.16,cheekRaiser_L:.12,cheekRaiser_R:.12})],
    mouth:[card('upper lip lifts',{mouthUpperUp_L:.20,mouthUpperUp_R:.20}),card('mouth stretches',{mouthStretch_L:.25,mouthStretch_R:.25}),card('lower lip drops',{mouthLowerDown_L:.20,mouthLowerDown_R:.20}),card('startled restraint',{mouthShrugUpper:.18,mouthPress_L:.08,mouthPress_R:.08})]
  }
};
let currentFamily='attentive',currentDraw={};const familyHistory=new Map();
...
function enableMorphControls(){document.querySelectorAll('.identity input,#jaw,#expressionStrength,#motionPace,#naturalBlink').forEach(el=>el.disabled=false);ready=true;drawFamily('attentive',false);setStatus(`READY / ${currentMask.toUpperCase()} MASK`,'ready');syncMorphs();}
const loader=new OBJLoader(),loadObj=name=>new Promise((res,rej)=>loader.load(FACEKIT+name,res,undefined,rej));
async function boot(){try{setStatus('LOADING NEUTRAL / ~2.6 MB');const base=await loadObj('generic_neutral_mesh.obj');prepareBase(base);for(let start=0;start<TARGETS.length;start+=4){const end=Math.min(start+4,TARGETS.length);setStatus(`LOADING MORPHS ${start+1}–${end}/${TARGETS.length}`);const roots=await Promise.all(TARGETS.slice(start,end).map(loadObj));roots.forEach((root,offset)=>attachTarget(root,start+offset));await new Promise(r=>requestAnimationFrame(r));}setStatus('BUILDING EXPRESSION DECK');await new Promise(r=>requestAnimationFrame(r));buildLayers();enableMorphControls();}catch(err){console.error(err);setStatus('LOAD FAILED — SEE CONSOLE','error');}}
...
function drawFamily(name=currentFamily,announce=true){
  currentFamily=FAMILY_DECKS[name]?name:'attentive';const deck=FAMILY_DECKS[currentFamily],recent=familyHistory.get(currentFamily)||[];let cards,signature;
  for(let attempt=0;attempt<16;attempt++){cards=[choose(deck.brows),choose(deck.eyes),choose(deck.mouth)];signature=cards.map(c=>c.label).join('|');if(!recent.includes(signature))break;}
  const jitter=.88+Math.random()*.24;currentDraw={};cards.forEach(c=>Object.entries(c.weights).forEach(([shape,value])=>{currentDraw[shape]=(currentDraw[shape]||0)+value*jitter;}));
  recent.unshift(signature);familyHistory.set(currentFamily,recent.slice(0,6));motionStarted=clock?.getElapsedTime?.()||0;
  document.querySelectorAll('[data-family]').forEach(btn=>{const active=btn.dataset.family===currentFamily;btn.classList.toggle('active',active);btn.setAttribute('aria-pressed',active?'true':'false');});
  const readout=document.querySelector('#dealReadout');if(readout)readout.textContent=cards.map(c=>c.label).join(' · ');
  if(announce&&ready)setStatus(`DEALT / ${currentFamily.toUpperCase()}`,'ready');
}
...
function addCurrentDraw(out,scale){
  Object.entries(currentDraw).forEach(([shape,value])=>{const index=MORPH_INDEX[shape];if(index!==undefined)out[index]+=value*scale;});
}
...
function buildExpressionTarget(t){
  ...
  if(motionMode==='hold')addCurrentDraw(expressionTarget,strength);
  if(motionMode==='breathe'){
    const cycle=.5-.5*Math.cos(age*Math.PI*2/13.5),envelope=.035+.965*smooth01(cycle);
    addCurrentDraw(expressionTarget,strength*envelope);
  }
  ...
  if(motionMode==='wander'){
    ...
    Object.entries(currentDraw).forEach(([shape,value],index)=>{
      const side=shape.endsWith('_R') ? .43 : 0,activity=.16+.84*wave(.075+(index%5)*.013,index*1.37+side,1.7+(index%3)*.3);
      put(shape,value*activity);
    });
  }
...
document.querySelectorAll('[data-family]').forEach(btn=>btn.addEventListener('click',()=>drawFamily(btn.dataset.family)));
document.querySelector('#dealAgain').addEventListener('click',()=>drawFamily(currentFamily));
