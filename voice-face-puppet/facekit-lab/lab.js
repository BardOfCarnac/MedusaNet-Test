...
import { buildGroupedGeometry, buildClusteredSkin, setMorphArray } from './facekit-geometry.js?v=11';
...
const excludedSurface=name=>helperMaterial(name)||name.includes('sclera')||name.includes('iris')||mouthInteriorMaterial(name)||name.includes('eyelashes');
const excludedContour=name=>helperMaterial(name)||mouthInteriorMaterial(name);
...
function buildSolidMaterial(sourceMaterial){
  ...
  if(helperMaterial(name)||mouthInteriorMaterial(name)){m.visible=false;m.userData.hiddenFromSolid=true;return m;}
  ...
  else if(name.includes('eyelashes')){m.color.setHex(0x090103);m.emissive.setHex(0);m.userData.opacityScale=.9;}
  return m;
}
...
function buildLayers(){
  baseMeshes.forEach((base,index)=>{
    ...
    const low=buildClusteredSkin(base,{portrait:portraitLayout(),excludedSurface,logLabel:'FaceKit Lab 11'});
    ...
    [depth,solid,mouthSoft,mouthTeeth,wire,contour].filter(Boolean).forEach(m=>{m.position.copy(base.position);m.rotation.copy(base.rotation);m.scale.copy(base.scale);holder.add(m);});
    headRig.add(holder);layerMeshes.push({solid,wire,contour,depth,mouthSoft,mouthTeeth});
  });
...
function syncMorphs(){const{identity,jaw}=morphValues();layerMeshes.forEach(({solid,wire,contour,depth,mouthSoft,mouthTeeth})=>[solid,wire,contour,depth,mouthSoft,mouthTeeth].forEach(m=>applyMorphs(m,identity,jaw)));updateMouthInterior();}
...
function updateLayerUI(){document.querySelector('#wireOut').value=pct(ui.wire.value);document.querySelector('#solidOut').value=pct(ui.solid.value);document.querySelector('#contourOut').value=pct(ui.contour.value);layerMeshes.forEach(({solid,wire,contour})=>{setSolidOpacity(solid,ui.solid.value);if(wire)wire.material.opacity=Number(ui.wire.value);if(contour)contour.material.uniforms.uOpacity.value=Number(ui.contour.value);});updateMouthInterior();}
...
function animate(){requestAnimationFrame(animate);resize();controls.update();const t=clock.getElapsedTime();if(ui.drift.checked&&ready){const w=.67+Math.sin(t*.23)*.11+Math.sin(t*.071)*.05,c=.24+Math.sin(t*.17+1.4)*.13,s=.10+Math.sin(t*.11+3.1)*.055;layerMeshes.forEach(({solid,wire,contour})=>{if(wire)wire.material.opacity=Math.max(.04,Math.min(1,w*Number(ui.wire.value)/.78));if(contour)contour.material.uniforms.uOpacity.value=Math.max(0,Math.min(1,c*Number(ui.contour.value)/.34));setSolidOpacity(solid,Math.max(0,Math.min(.5,s*Number(ui.solid.value)/.14)));});updateMouthInterior();}else updateLayerUI();headRig.rotation.y=Math.sin(t*.19)*.018;renderer.render(scene,camera);}
