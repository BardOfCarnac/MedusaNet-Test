...
const VISEMES={
  ...
  O:{mouthPucker:1,jawForward:.60,jawOpen:.20},U:{mouthFunnel:1},
  ...
  TH:{mouthRollUpper:.60,jawOpen:.20},CH:{mouthPucker:.50,jawOpen:.20},RR:{mouthPucker:.50,jawOpen:.20},
...
const C=.095,V=.18;
const WORDS=[
 ['Father',[['f','FF',C,ANCHOR],['ɑː','aa',V,ANCHOR],['ð','TH',C,SECONDARY,.34,{mouthRollLower:.24,mouthDimple_L:.10,mouthDimple_R:.10}],['ə','E',.13,SECONDARY,.28,{mouthRollLower:.10,mouthDimple_L:.05,mouthDimple_R:.05}],['r','RR',C,SECONDARY,.36,{mouthPucker:.10,mouthRollLower:.08}]]],
 ...
 ['in',[['ɪ','I',.14,HOLD],['n','nn',C,HOLD]]],['a',[['ə','E',.13,HOLD]]],
...
// Windows measured from the saved 8.34 s recording. Real pauses are retained.
const WORD_TIMES={Father:[.70,1.06],packed:[1.10,1.44],five:[1.50,1.85],bright:[1.95,2.27],blue:[2.31,2.61],puppets:[2.64,2.99],in:[3.00,3.18],a:[3.18,3.34],good:[3.34,3.55],box:[3.60,3.83],then:[4.63,4.92],Joe:[4.93,5.22],chose:[5.24,5.65],three:[5.65,5.97],sheep:[5.98,6.24],by:[6.43,6.60],the:[6.60,6.70],old:[6.70,6.89],gate:[6.98,7.19]};
const CLIP_END=8.34,WORD_WINDOWS=Object.values(WORD_TIMES);
...
const SEGMENTS=[];
for(const [word,seq] of WORDS){const [ws,we]=WORD_TIMES[word],weights=seq.map(e=>e[2]),sum=weights.reduce((a,b)=>a+b,0);let t=ws;for(let i=0;i<seq.length;i++){const [phoneme,viseme,w,visual,weight,overlay]=seq[i],end=i===seq.length-1?we:t+(we-ws)*w/sum,profile=gestureProfile(viseme,visual,word,phoneme);SEGMENTS.push({word,phoneme,viseme,visual,weight,overlay,start:t,end,center:(t+end)/2-profile.anticipation,...profile});t=end;}}
...
const scene=new THREE.Scene();scene.fog=new THREE.FogExp2(0x050507,.012);const camera=new THREE.PerspectiveCamera(34,1,.1,500);camera.position.set(0,1.5,43);const controls=new OrbitControls(camera,canvas);controls.enableDamping=true;controls.target.set(0,1.4,2.4);controls.enablePan=false;controls.minDistance=24;controls.maxDistance=75;const key=new THREE.DirectionalLight(0xff1d31,6.5);key.position.set(-7,22,15);scene.add(key);scene.add(new THREE.AmbientLight(0x380006,.055));
const faceMat=new THREE.MeshStandardMaterial({color:0xb40a19,roughness:.92,metalness:0}),darkMat=new THREE.MeshBasicMaterial({color:0x030001}),mouthMat=new THREE.MeshStandardMaterial({color:0x250307,roughness:1}),teethMat=new THREE.MeshStandardMaterial({color:0x7d343b,roughness:.9});
const loader=new OBJLoader();let baseRoot=null,baseMeshes=[],ready=false;const current=new Float32Array(TARGETS.length),target=new Float32Array(TARGETS.length);
...
function rawPoseAt(t){
  const out=new Float32Array(TARGETS.length),lipNum=new Float32Array(TARGETS.length),jawNum=new Float32Array(TARGETS.length),inWord=isInsideWord(t);let lipDen=inWord?.055:1.10,jawDen=inWord?.05:1.25;
  for(const e of SEGMENTS){const k=smoothKernel(t,e.center,e.radius);if(k<=0)continue;const recipe=BASE_VECTORS[e.viseme]||BASE_VECTORS.sil,scale=recipeScale(e.viseme),base=k*e.priority,lw=base*e.lip*(e.visual===SECONDARY?(e.weight??.28)/.28:1),jw=base*e.jaw*(e.visual===SECONDARY?Math.max(.7,(e.weight??.28)/.28):1);lipDen+=lw;jawDen+=jw;for(const name of LIPS){const i=INDEX[name];lipNum[i]+=recipe[i]*scale*lw;}for(const name of JAW){const i=INDEX[name];jawNum[i]+=recipe[i]*scale*jw;}if(e.overlay){const ow=k*masterArticulation();for(const [name,value] of Object.entries(e.overlay)){const i=INDEX[name];if(i===undefined)continue;if(JAW.has(name))jawNum[i]+=value*ow*jw;else lipNum[i]+=value*ow*lw;}}}
  for(const name of LIPS){const i=INDEX[name];out[i]=THREE.MathUtils.clamp(lipNum[i]/lipDen,0,1.15);}for(const name of JAW){const i=INDEX[name];out[i]=THREE.MathUtils.clamp(jawNum[i]/jawDen,0,1.15);}return out;
}
const LIP_HOLD={start:2.985,end:3.335,blend:.055};
function poseAt(t){const out=rawPoseAt(t);if(t>=LIP_HOLD.start-LIP_HOLD.blend&&t<=LIP_HOLD.end+LIP_HOLD.blend){const hold=rawPoseAt(LIP_HOLD.start-.012);let amount=1;if(t<LIP_HOLD.start)amount=THREE.MathUtils.smoothstep(t,LIP_HOLD.start-LIP_HOLD.blend,LIP_HOLD.start);else if(t>LIP_HOLD.end)amount=1-THREE.MathUtils.smoothstep(t,LIP_HOLD.end,LIP_HOLD.end+LIP_HOLD.blend);for(const name of LIPS){const i=INDEX[name];out[i]=THREE.MathUtils.lerp(out[i],hold[i],amount);}}return out;}
function segmentAt(t){let best=null;for(const e of SEGMENTS){if(t>=e.start&&t<e.end)return e;if(!best||Math.abs(t-e.center)<Math.abs(t-best.center))best=e;}return best;}
let timelineTime=0,playing=false,lastNow=performance.now(),selectedIndex=0,external=false;
function updateReadout(t,forced=null){if(forced){document.querySelector('#word').textContent='MANUAL';document.querySelector('#phoneme').textContent='—';document.querySelector('#viseme').textContent=forced;document.querySelector('#visual').textContent='FORCED REFERENCE';return;}const e=segmentAt(t);if(e)selectedIndex=Math.max(0,SEGMENTS.indexOf(e));if(!e||t<WORD_TIMES.Father[0]||t>WORD_TIMES.gate[1]){document.querySelector('#word').textContent='—';document.querySelector('#phoneme').textContent='silence';document.querySelector('#viseme').textContent='SIL';document.querySelector('#visual').textContent='REST';return;}document.querySelector('#word').textContent=e.word;document.querySelector('#phoneme').textContent=e.phoneme;document.querySelector('#viseme').textContent=e.viseme;document.querySelectorAll('[data-viseme]').forEach(b=>b.classList.toggle('active',b.dataset.viseme===e.viseme));const prep=Math.round(e.anticipation*1000),kind=e.visual===ANCHOR?'ANCHOR':e.visual===SECONDARY?'OVERLAP':'INTERNAL';document.querySelector('#visual').textContent=`${kind} · PREP ${prep}ms`;}
...
document.querySelector('#play').addEventListener('click',play);document.querySelector('#pause').addEventListener('click',pause);document.querySelector('#prev').addEventListener('click',()=>step(-1));document.querySelector('#next').addEventListener('click',()=>step(1));document.querySelector('#speed').addEventListener('input',e=>{document.querySelector('#speedOut').value=Math.round(Number(e.target.value)*100)+'%';});document.querySelector('#articulation').addEventListener('input',e=>{document.querySelector('#articulationOut').value=Math.round(Number(e.target.value)*100)+'%';setTimelineTime(timelineTime);});
window.goldSpeech={get ready(){return ready;},get time(){return timelineTime;},get clipEnd(){return CLIP_END;},setExternalTime(t){external=true;playing=false;setTimelineTime(t);},releaseExternal(){external=false;},pause(){pause();},reset(){external=false;pause();selectedIndex=0;setTimelineTime(0);}};

async function boot(){try{status.textContent='LOADING NEUTRAL HEAD…';baseRoot=await loadObj('generic_neutral_mesh.obj');prepare(baseRoot);scene.add(baseRoot);frameModel();status.textContent='HEAD READY · LOADING SPEECH SHAPES…';for(let i=0;i<TARGETS.length;i++){status.textContent=`HEAD READY · SPEECH SHAPE ${i+1}/${TARGETS.length} · ${TARGETS[i]}`;attach(await loadObj(TARGETS[i]+'.obj'),i);await new Promise(r=>requestAnimationFrame(r));}baseMeshes.forEach(m=>m.updateMorphTargets());ready=true;document.querySelectorAll('button').forEach(b=>b.disabled=false);status.textContent='READY · RECORDED TIMING / BIDIRECTIONAL CO-ARTICULATION';setTimelineTime(0);window.dispatchEvent(new Event('goldspeechready'));}catch(err){console.error(err);status.textContent='LOAD FAILED · '+err.message;}}
...
function animate(now){requestAnimationFrame(animate);resize();controls.update();const dt=Math.min(.05,(now-lastNow)/1000);lastNow=now;if(playing&&!external){const speed=Number(document.querySelector('#speed').value)||.55;timelineTime+=dt*speed;if(timelineTime>=CLIP_END){timelineTime=CLIP_END;pause();}setTimelineTime(timelineTime);}const speed=Number(document.querySelector('#speed').value)||.55,tau=.045,alpha=1-Math.exp(-Math.max(.001,dt*speed)/tau);let moving=false;for(let i=0;i<current.length;i++){const n=THREE.MathUtils.lerp(current[i],target[i],alpha);if(Math.abs(n-current[i])>.0001)moving=true;current[i]=n;}if(moving)applyMorphs();renderer.render(scene,camera);}
