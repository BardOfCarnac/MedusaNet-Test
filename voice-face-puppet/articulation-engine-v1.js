...
export class SpeechArticulationEngine{
  ...
  update({vowelWeights={},volume=0,haWeights={},rawActive='sil',gate=null,vadDb=-100,dtMs=16}={}){
    const dtSec=Math.max(.001,dtMs/1000),level=clamp((+volume||0)*1.18),speechTarget=(level>.018||vadDb>-55)?1:0;
    this.speechEnv+= (speechTarget-this.speechEnv)*alpha(dtMs,speechTarget>this.speechEnv?22:145);
    ...
    if(this.useConsonants){
      for(const name of CONSONANT_ORDER){
        ...
        const env=this.consonantEnv[name],a=alpha(dtMs,wanted>env?p.attack:p.release);this.consonantEnv[name]=env+(wanted-env)*a;
      ...
      for(const k of CONSONANT_ORDER)this.consonantEnv[k]+=(0-this.consonantEnv[k])*alpha(dtMs,85);
    }
    ...
    return {morphs,shape,detail,level,speechEnvelope:this.speechEnv,weights:normalized,overlay,articulation:{mode:'continuous-coarticulation',jawSeparate:true,lipSeparate:true,predictiveLead:true,allConsonants:this.useConsonants}};
  }
}
