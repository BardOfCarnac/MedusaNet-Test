...
function setModel(m,persist=true){
  ...
  $('directBtn').disabled=false;$('modelReadout').textContent=`MY VOICE · ${personalModel.classifier.classes.join(' / ')} · ${personalModel.feature.contextFrames*10} ms context`;
  selectMode('direct');
}
const saved=readSavedModel();if(saved){try{setModel(saved,false)}catch(e){console.warn(e)}}
$('modelFile').addEventListener('change',async e=>{const f=e.target.files?.[0];if(!f)return;try{setModel(JSON.parse(await f.text()),true);$('micStatus').textContent='Personal direct model loaded and saved locally.'}catch(err){$('micStatus').textContent=err.message}finally{e.target.value=''}});
...
// ---------- FaceKit 3D ----------
const canvas=$('stage'),renderer=new THREE.WebGLRenderer({canvas,antialias:true,alpha:false});renderer.setPixelRatio(Math.min(devicePixelRatio,2));renderer.setClearColor(0x040406,1);renderer.outputColorSpace=THREE.SRGBColorSpace;
const scene=new THREE.Scene();scene.fog=new THREE.FogExp2(0x050507,.014);const portrait=()=>innerWidth<=700||innerHeight>innerWidth*1.1;const camera=new THREE.PerspectiveCamera(34,1,.1,500);camera.position.set(0,portrait()?4:-.3,portrait()?48:43);
...
const loader=new OBJLoader(),loadObj=name=>new Promise((resolve,reject)=>loader.load(FACEKIT+name,resolve,undefined,reject));const meshList=root=>{const a=[];root.traverse(o=>{if(o.isMesh)a.push(o)});return a},materialsOf=m=>Array.isArray(m)?m:[m];let baseMeshes=[],renderMeshes=[],faceReady=false;
...
// ---------- microphone + A/B ----------
let wlLib=null,stockWl=null,wlProfile=null;const wlReady=Promise.all([import(WL_URL),fetch(WL_PROFILE).then(r=>{if(!r.ok)throw new Error('wLip profile '+r.status);return r.json()})]).then(([m,p])=>{wlLib=m;stockWl=p;try{wlProfile=JSON.parse(localStorage.getItem(WL_STORAGE)||'null')||p}catch{wlProfile=p}}).catch(e=>console.error(e));
let audioCtx=null,stream=null,source=null,script=null,zeroGain=null,directStream=null,wlNode=null,mic=false,mode=personalModel?'direct':'wl';let directState={weights:{A:0,E:0,I:0,O:0,U:0},db:-100,confidence:0,shape:'REST'};
...
let morph=blankMorphs();function setBar(id,v){const e=$(id);if(e)e.style.width=`${clamp(v)*100}%`}
...
const clock=new THREE.Clock();function animate(){requestAnimationFrame(animate);resize();controls.update();const s=mode==='direct'?directSample():wlSample();applySample(s);const t=clock.getElapsedTime();headRig.rotation.y=$('drift').checked?Math.sin(t*.19)*.018:0;renderer.render(scene,camera)}
